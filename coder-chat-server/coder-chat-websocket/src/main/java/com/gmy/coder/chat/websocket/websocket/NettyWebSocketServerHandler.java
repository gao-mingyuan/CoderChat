...
//    private WebSocketService webSocketService;
    ...
    Map<Channel, String> map = new ConcurrentHashMap<>();

    // 当web客户端连接后，触发该方法
    @Override
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
//        this.webSocketService = getService();
        log.info("连接成功,IP:{}", InetAddress.getLocalHost().getHostAddress());
    }
...
//            this.webSocketService.connect(ctx.channel());
            ...
            map.put(ctx.channel(), InetAddress.getLocalHost().getHostAddress());
...
//                this.webSocketService.authorize(ctx.channel(), new WSAuthorize(token));
    ...
    protected void channelRead0(ChannelHandlerContext ctx, TextWebSocketFrame msg) throws Exception {
        ctx.writeAndFlush(new TextWebSocketFrame("当前在线人数:" + map.size() + ",当前服务器ip:" + InetAddress.getLocalHost().getHostAddress()));
    }
}
